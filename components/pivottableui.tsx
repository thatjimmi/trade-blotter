...
const TableConfigPanel = ({ 
  ...
  <div className="p-4 border border-gray-800 rounded-lg bg-[#0B0B0F]/50 space-y-4">
    ...
    <div className="space-y-4">
      <div>
        <label className="block text-xs mb-1 text-gray-400">Column Dimensions</label>
        <MultiSelect
          options={dimensionOptions}
          value={config.colDimensions}
          onChange={(values) => onUpdate(config.id, { ...config, colDimensions: values })}
          placeholder="Select column dimensions"
        />
      </div>

      <div>
        <label className="block text-xs mb-1 text-gray-400">Value Dimension</label>
        <MultiSelect
          options={numericOptions}
          value={[config.valueDimension]}
          onChange={(values) => onUpdate(config.id, { ...config, valueDimension: values[0] })}
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => onUpdate(config.id, { ...config, formatType: e.target.value })}
...
const MultiSelect = ({ options, value, onChange, placeholder }) => (
  <select 
    ...
    className="w-full p-2 rounded-lg bg-[#0B0B0F] text-gray-300 border border-gray-800 
      focus:outline-none focus:ring-2 focus:ring-gray-700"
  >
    <option value="" disabled className="bg-[#0B0B0F]">{placeholder}</option>
    {options.map(opt => (
...
const PivotTableUI = ({ data, initialConfig }) => {
  ...
  return (
    <div className="w-full space-y-6 bg-[#0B0B0F] text-gray-300 p-6 rounded-xl">
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-sm font-medium text-gray-400">Table Configurations</h3>
            <button 
              onClick={addNewTable}
                ...
                {headerRow.map((header, colIndex) => (
                  <th
                    ...
                    className={`
                      ...
                      ${header.isRowHeader ? 'sticky left-0 bg-[#0B0B0F] z-10 border-r border-gray-800' : ''}
